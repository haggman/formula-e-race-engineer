...
> source activate.sh # finalize the environmental variables
...
| DEPLOYED wall: SIM bar shows `sim: {"detail":"SIM_URL not configured"}` | frontend deployed while sim-URL discovery transiently failed — empty SIM_URL baked into the service env (Finding #11 addendum) | no rebuild needed: `gcloud run services update fe-frontend --region=$REGION --update-env-vars=SIM_URL=$(gcloud run services describe fe-simulator --region=$REGION --format='value(status.url)')` — deploy script now retries + hard-fails instead |
